...
```bash
npm install @jsmart/jsmart-ai
```

TypeBox exports are re-exported from `@jsmart/jsmart-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@jsmart/jsmart-ai';
...
```typescript
import { Type, Tool, StringEnum } from '@jsmart/jsmart-ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@jsmart/jsmart-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@jsmart/jsmart-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@jsmart/jsmart-ai';
...
```typescript
import { getModel, stream } from '@jsmart/jsmart-ai';
...
```typescript
import { getProviders, getModels, getModel } from '@jsmart/jsmart-ai';
...
```typescript
import { Model, stream } from '@jsmart/jsmart-ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@jsmart/jsmart-ai';
...
```typescript
import { getModel, complete, Context } from '@jsmart/jsmart-ai';
...
```typescript
import { Context, getModel, complete } from '@jsmart/jsmart-ai';
...
- OAuth login flows are not supported in browser environments. Use the `@jsmart/jsmart-ai/oauth` entry point in Node.js.
...
Set `JSMART_AI_ANTIGRAVITY_VERSION` to override the Antigravity User-Agent version when Google updates their requirements:

```bash
export JSMART_AI_ANTIGRAVITY_VERSION="1.23.0"
```

#### Cache Retention

Set `JSMART_CACHE_RETENTION=long` to extend prompt cache retention:

| Provider | Default | With `JSMART_CACHE_RETENTION=long` |
|----------|---------|-------------------------------|
...
```typescript
import { getEnvApiKey } from '@jsmart/jsmart-ai';
...
```bash
npx @jsmart/jsmart-ai login              # interactive provider selection
npx @jsmart/jsmart-ai login anthropic    # login to specific provider
npx @jsmart/jsmart-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@jsmart/jsmart-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@jsmart/jsmart-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@jsmart/jsmart-ai/oauth';
...
```typescript
import { getModel, complete } from '@jsmart/jsmart-ai';
import { getOAuthApiKey } from '@jsmart/jsmart-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@jsmart/jsmart-ai`
