# @jsmart/jsmart-agent-core

Stateful agent with tool execution and event streaming. Built on `@jsmart/jsmart-ai`.

## Installation

```bash
npm install @jsmart/jsmart-agent-core
```
...
import { getModel } from "@jsmart/jsmart-ai";
...
```typescript
declare module "@jsmart/jsmart-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@jsmart/jsmart-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@jsmart/jsmart-agent-core";
