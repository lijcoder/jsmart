...
export async function downloadMediaFromItem(
	...
	if (item.type === MessageItemType.IMAGE) {
		...
		try {
			...
			result = { type: "file", filePath: saved.path, fileName: generalFileName() };
		...
		if ((!voice?.media?.encrypt_query_param && !voice?.media?.full_url) || !voice?.media?.aes_key)
			return { success: false, error: "no download URL or AES key" };
		try {
			...
			if (wavBuf) {
				...
				result = { type: "file", filePath: saved.path, fileName: `${generalFileName()}.wav` };
				...
				result = { type: "file", filePath: saved.path, fileName: `${generalFileName()}.silk` };
		...
		try {
			...
			result = { type: "file", filePath: saved.path, fileName: fileItem.file_name ?? generalFileName() };
		...
		try {
			...
			result = { type: "file", filePath: saved.path, fileName: `${generalFileName()}.mp4` };
...
function generalFileName(): string {
	return randomUUID();
}
