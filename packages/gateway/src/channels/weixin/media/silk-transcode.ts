...
const _SILK_SAMPLE_RATE = 24_000;

/**
 * Wrap raw pcm_s16le bytes in a WAV container.
 * Mono channel, 16-bit signed little-endian.
 */
function _pcmBytesToWav(pcm: Uint8Array, sampleRate: number): Buffer {
 ...
 */
export async function silkToWav(_silkBuf: Buffer): Promise<Buffer | null> {
	try {
