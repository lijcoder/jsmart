...
import type { AssistantMessage, Model, Usage } from "@jsmart/jsmart-ai";
...
function loadLargeSession(): SessionManager {
	const sessionPath = join(__dirname, "fixtures/large-session.jsonl");
	const session = new SessionManager(true, sessionPath);
	return session;
}
...
describe("Large session fixture", () => {
	it("should parse the large session", () => {
		const session = loadLargeSession();
	...
	it("should load session correctly", () => {
		const session = loadLargeSession();
		const loaded = session.buildSessionContext();
...
describe.skipIf(!process.env.JSMART_BASE_URL || !process.env.JSMART_MODEL || !process.env.JSMART_API_KEY)(
	...
	() => {
		...
		it("should generate a compaction result for the large session", async () => {
			...
			const compactionResult = await compact(entries, model, process.env.JSMART_API_KEY!);
